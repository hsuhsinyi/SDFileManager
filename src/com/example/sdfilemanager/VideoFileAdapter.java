...
public class VideoFileAdapter extends SimpleAdapter{
	...
	public VideoFileAdapter(Context context, List<Map<String, Object>> data,
...
//		mInflater = LayoutInflater.from(context);  //context = this.getActivity();
//		mList = data;     //a1或者a2
//		for(int i = 0; i < data.size(); i++) {
//            map.put(i, false);    //设置全部false
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if(convertView == null){
			...
			holder.listImageView = (ImageView) convertView.findViewById(R.id.imageDir);
			holder.listFileName = (TextView) convertView.findViewById(R.id.name);
			holder.listMdTime = (TextView) convertView.findViewById(R.id.mdtime);
			holder.listcheckbox = (CheckBox)convertView.findViewById(R.id.listcheckbox);
			
		}else{
			holder = (ViewHolder) convertView.getTag();
		}
	    convertView.setTag(holder);
		holder.listImageView.setBackgroundResource((Integer) mlist.get(position).get("image"));
		holder.listFileName.setText(mlist.get(position).get("name").toString());
		holder.listMdTime.setText(mlist.get(position).get("modifytime").toString());
//		holder.listcheckbox.setOnClickListener(new OnClickListener() {
//			
//			@Override
//			public void onClick(View v) {
//				// TODO Auto-generated method stub
//				CheckBox listcheckbox = (CheckBox)v;
